...
public interface EcoleRepository extends JpaRepository<Ecole, Integer>
{
	...
	@Query(value="select ec.* from etudiant e, ecole ec where e.id_ecole = ec.id and e.nom=:nom and e.prenom=:prenom", nativeQuery = true)
	Ecole findSchoolByNomEtPrenom(@Param("nom") String nom, @Param("prenom") String prenom);
}
